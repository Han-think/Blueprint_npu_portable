...
## 0. 원샷 복붙 스크립트 (vLLM, 2026-06-24 검증)

> Pod 생성 후 터미널 열고 아래를 **순서대로 복붙**하면 끝.
> 템플릿: **RunPod PyTorch 2.4.0** / GPU: **A40 48GB**
> 디스크: **Container 60GB + Volume 0GB** (모델 캐시 휘발 OK — 재다운 10분)

### 방법 A: 셋업 스크립트 (추천)

```bash
cd /workspace
git clone -b update-runpod-guide https://github.com/Han-think/Blueprint_ipex_portable.git
cd Blueprint_ipex_portable
bash setup_runpod.sh
# "vLLM 서버 준비 완료!" 뜨면 아래 배치 명령 실행
```
...
# ── Step 3: pyairports stub (필수! pip 0.0.1에 airports.py 없음) ──
# pip install pyairports 하면 안 됨 — 성공하지만 모듈 구조가 깨져있음
SITE=$(python -c "import site; print(site.getsitepackages()[0])")
mkdir -p "$SITE/pyairports"
printf 'airport_list=[]\nAIRPORT_LIST=[]\n' > "$SITE/pyairports/__init__.py"
...
BP_WORKERS=12 \
BP_SKIP_AUDIT=1 \
nohup python generate_batch.py --seeds all --per-seed 30 \
...
```bash
# ── Step 9: 완료 후 Git push ──
cd /workspace/Blueprint_ipex_portable
git add 30_model/curation/
git commit -m "batch3: 180 candidates via enriched packs + vLLM A40"
git push
```
...
| 디스크 | 마운트 | 용도 | 권장 크기 | 유지 |
|--------|--------|------|----------|------|
| Container Disk | `/` | OS, pip, CUDA, 모델 캐시 | **60GB** | Pod 삭제 시 사라짐 |
| Network Volume | `/workspace` | (선택) 코드, 데이터 | **0GB** (불필요) | Pod 재시작해도 유지 |

> **권장: Container 60GB / Volume 0GB.** 모델 재다운 10분이면 끝나고, Volume 과금($0.006/hr) 없음.
> 결과는 git push로 보존. Volume은 자주 Pod 재시작하는 경우에만 고려.
