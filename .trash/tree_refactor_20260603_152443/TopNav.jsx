...
const BP_PRIMARY = [
  ...
  { k:'Model',     href:'Setup.html',     label:'모델',       icon:'◉' },
...
function BPHealth() {
    ...
    const ping = () => {
      ...
      fetch('http://127.0.0.1:11434/api/tags', { signal: ctrl.signal })
        ...
        .then(data => {
          ...
          setState('online');
          setInfo(ms.length === 0 ? 'no models'
                : ms.length === 1 ? ms[0].name
                : `${ms[0].name} +${ms.length - 1}`);
        })
  ...
  const map = {
    ...
    online:   { c: BP_C.inter, dot: BP_C.inter, t: `ollama · ${info}` },
    offline:  { c: BP_C.risk,  dot: BP_C.risk,  t: 'ollama offline' },
  }[state];
