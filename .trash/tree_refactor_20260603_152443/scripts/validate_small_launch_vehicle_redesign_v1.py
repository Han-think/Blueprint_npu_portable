...
PACKAGE_PATH = ROOT / "data" / "blueprint" / "small_launch_vehicle_redesign_package_v1.json"
DATASET_PATH = ROOT / "data" / "blueprint" / "small_launch_vehicle_design_thinking_v1.jsonl"
...
def validate_design_package():
    ...
    require(package["target"]["vehicle_id"] == "small_launch_vehicle", "target vehicle must be small_launch_vehicle")
    require(package["primary_metric"]["name"] == "assembly_serviceability_flowpath_readability", "primary metric mismatch")
    ...
    for keyword in ["engine", "tank", "interstage", "fairing", "service bay", "flow-path"]:
    ...
    require("flow path" in risk_text, "flow path readability risk required")
    require("safety boundary" in risk_text, "inert educational boundary risk required")

    package_text = json.dumps(package, ensure_ascii=False).lower()
    for blocked in ["propellant formula", "ignition instructions", "pressure vessel design", "launch operations"]:
...
def validate_dataset():
    ...
    with DATASET_PATH.open("r", encoding="utf-8") as f:
        for line_no, line in enumerate(f, 1):
            ...
            require(row["target"] == "small_launch_vehicle", f"line {line_no}: target mismatch")
            require(row["metric"] == "assembly_serviceability_flowpath_readability", f"line {line_no}: metric mismatch")
    ...
    require("risk_flow_hidden" in classes, "dataset must include hidden-flow risk examples")

    dataset_text = "\n".join(json.dumps(row, ensure_ascii=False) for row in rows).lower()
    for keyword in ["engine", "tank", "interstage", "fairing", "service bay", "flow", "cutaway"]:
...
def main():
    ...
    print("Small launch vehicle redesign V1 validation OK")
