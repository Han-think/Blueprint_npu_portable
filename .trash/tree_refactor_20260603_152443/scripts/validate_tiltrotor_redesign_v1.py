...
PACKAGE_PATH = ROOT / "data" / "blueprint" / "tiltrotor_redesign_package_v1.json"
DATASET_PATH = ROOT / "data" / "blueprint" / "tiltrotor_design_thinking_v1.jsonl"
...
def validate_design_package():
    ...
    require(package["target"]["vehicle_id"] == "tiltrotor_vtol", "target vehicle must be tiltrotor_vtol")
    require(package["primary_metric"]["name"] == "assembly_serviceability_efficiency", "primary metric mismatch")
    require(len(package["existing_bom"]) >= 8, "existing BOM must contain at least 8 groups")
    ...
    require(len(package["assembly_delta"]["retained_service_access_points"]) >= 4, "at least 4 service access points required")
    ...
    for keyword in ["nacelle", "tilt-axis", "battery", "wiring"]:
    ...
    require("tilt-axis alignment" in risk_text, "tilt-axis alignment risk required")
...
def validate_dataset():
    ...
    with DATASET_PATH.open("r", encoding="utf-8") as f:
        for line_no, line in enumerate(f, 1):
            ...
            require(row["target"] == "tiltrotor_vtol", f"line {line_no}: target mismatch")
    ...
    for keyword in ["nacelle", "tilt", "battery", "wiring"]:
...
def main():
    ...
    print("Tiltrotor redesign V1 validation OK")
