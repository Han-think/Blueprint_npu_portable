...
TARGET = "haptic_glove_pair"
METRIC = "wearable_serviceability_efficiency"
...
def validate_design_package():
    ...
    require(package["target"]["vehicle_id"] == TARGET, f"target vehicle must be {TARGET}")
    require(package["primary_metric"]["name"] == METRIC, "primary metric mismatch")
    ...
    require(any("service access" in v["check"] for v in bp["verify"]), "serviceability verify check required")
    return package
...
def validate_dataset():
    ...
    with DATASET_PATH.open("r", encoding="utf-8") as f:
        for line_no, line in enumerate(f, 1):
            ...
            require(row["target"] == TARGET, f"line {line_no}: target mismatch")
            require(row["metric"] == METRIC, f"line {line_no}: metric mismatch")
...
def main():
    ...
    print("Haptic exo-glove redesign V1 validation OK")
